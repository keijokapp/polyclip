 ...
 * @param {import('./index.js').Geometry[]} geometries
 * @returns {import('./index.js').MultiPolygon}
 */
export default function runOperation(type, geometries) {
	...
	if (type === 'difference') {
		...
		while (i < multipolygons.length) {
			if (getBboxOverlap(getBoundingBox(multipolygons[i]), subjectBbox) != null) {
				i++;
			} else {
				multipolygons.splice(i, 1);
			}
		}
	}
