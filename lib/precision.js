...
function orient(eps) {
	...
	return (a, b, c) => {
		...
		return /** @type {number} */(area2.comparedTo(0));
	};
}
...
/**
 * @param {number} [eps]
 * @returns {(v: import('./vector.js').Vector) => import('./vector.js').Vector}
 */
function snap(eps) {
	if (eps) {
...
function compare(eps) {
	 ...
	 */
	return (a, b) => {
		if (eps && b.minus(a).abs().isLessThanOrEqualTo(eps)) {
			return 0;
		}

		return /** @type {number} */(a.comparedTo(b));
	};
}
