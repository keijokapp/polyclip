...
export default function renderMultipolygon(segments, type, multipolygonCount) {
	...
	return segmentsToRings(segments, context)
		...
		.map(polygon => {
			if (polygon?.[0] != null) {
				return polygon.filter(
					/**
					 * @param {import('./index.js').Ring | undefined} ring
					 * @returns {ring is import('./index.js').Ring}
					 */
					ring => ring != null
				);
			}
		...
		.filter(
			/**
			 * @param {import('./index.js').Polygon | undefined} polygon
			 * @returns {polygon is import('./index.js').Polygon}
			 */
