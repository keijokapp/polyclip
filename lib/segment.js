...
export default class Segment {
	 ...
	 * @param {import('./sweep-event.js').Point | import('./vector.js').Vector} pointOrVector
	 * @returns {SweepEvent[]}
	 */
	split(pointOrVector) {
		const newEvents = [];
	...
	prevInResult(context) {
		if (this._prevInResult === undefined) {
			...
			} else if (this.prev.isInResult(context)) {
				this._prevInResult = this.prev;
			} else {
				this._prevInResult = this.prev.prevInResult(context) ?? null;
			}
		}
