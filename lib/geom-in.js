...
/**
 * @param {import('./sweep-event.js').Point} point1
 * @param {import('./sweep-event.js').Point} point2
 * @param {import('./geom-in.js').RingIn} ring
 * @returns {Segment}
 */
function segmentFromRing(point1, point2, ring) {
	/** @type {import('./sweep-event.js').Point} */
	let leftPoint;
	/** @type {import('./sweep-event.js').Point} */
	let rightPoint;
	/** @type {import('./segment.js').Winding} */
	let winding;

	// ordering the two points according to sweep line ordering
	const cmpPts = SweepEvent.comparePoints(point1, point2);
	if (cmpPts < 0) {
		leftPoint = point1;
		rightPoint = point2;
		winding = 1;
	...
	const leftSE = new SweepEvent(leftPoint, true);
	const rightSE = new SweepEvent(rightPoint, false);
		...
		polyin.rings = polygon.map((ring, i) => {
			...
			for (let i = 1, iMax = ring.length; i < iMax; i++) {
				...
				if (!point.x.eq(prevPoint.x) || !point.y.eq(prevPoint.y)) {
					ringin.segments.push(segmentFromRing(
						prevPoint,
						point,
						ringin
					));
