...
/**
 * @param {import('./index.js').MultiPolygon<import('./vector.js').Vector>} mulitpolygon
...
/**
 * @param {import('./index.js').Geometry<[number, number]>} geometry
 * @returns {import('./index.js').MultiPolygon<import('./vector.js').Vector>}
 */
export function normalizePolygon(geometry) {
	...
	try {
		...
		if (typeof geometry[0][0][0] === 'number') {
			geometry = [/** @type {import('./index.js').Polygon} */(geometry)];
		}
	} catch (ex) {
	...
	return /** @type {import('./index.js').MultiPolygon} */(geometry).map(polygon => {
